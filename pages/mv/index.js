...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.request({
      ...
      success: function(res) {
        wx.getNetworkType({
          complete: function(r) {
            let src = res.data.data.brs
            let wifi = r.networkType != 'wifi' ? false : true
            let url = src[1080] || src[720] || src[480] || src[240]
  ...
  loadmore: function() {
    if (this.data.rec.more && !this.data.loading) {
      ...
      common.loadrec(app.globalData.cookie, this.data.offset, this.data.limit, this.data.recid, function(data) {
        let rec = that.data.rec
        let offset = that.data.offset + data.comments.length
